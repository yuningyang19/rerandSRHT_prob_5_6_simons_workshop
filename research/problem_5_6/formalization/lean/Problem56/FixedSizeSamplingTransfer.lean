...
/-!
Transfer from the two Bernoulli sampling laws to uniform sampling without
replacement.  The proof uses the finite nested coupling and the deterministic
spectral sandwich.
-/
    ...
    uniformExpectation (fun ω ↦
        ...
        uniformExpectation (fun ω ↦
          bernoulliProbability θminus (minusBad ω) +
            bernoulliProbability θplus (plusBad ω) +
            bernoulliProbability θminus minusCountBad +
            bernoulliProbability θplus plusCountBad) :=
      uniformExpectation_mono _ _ hpoint
    _ = uniformBernoulliFailureProbability θminus (ε / 4) X +
          uniformBernoulliFailureProbability θplus (ε / 4) X +
          bernoulliProbability θminus minusCountBad +
          bernoulliProbability θplus plusCountBad := by
      rw [uniformExpectation_add, uniformExpectation_add,
        uniformExpectation_add, uniformExpectation_const,
        uniformExpectation_const]
      rfl
    _ ≤ γ + γ + Real.exp (-(ε ^ 2 * k / 48)) +
          Real.exp (-(ε ^ 2 * k / 48)) := by
      have hminus' :
          uniformBernoulliFailureProbability θminus (ε / 4) X ≤ γ := by
        simpa only [θminus] using hminus
      have hplus' :
          uniformBernoulliFailureProbability θplus (ε / 4) X ≤ γ := by
        simpa only [θplus] using hplus
      have htailMinus : bernoulliProbability θminus minusCountBad ≤
          Real.exp (-(ε ^ 2 * k / 48)) := by
        simpa only [θminus, minusCountBad] using htail.1
      have htailPlus : bernoulliProbability θplus plusCountBad ≤
          Real.exp (-(ε ^ 2 * k / 48)) := by
        simpa only [θplus, plusCountBad] using htail.2
      linarith
    _ = 2 * γ + 2 * Real.exp (-(ε ^ 2 * k / 48)) := by ring

#print axioms fixed_size_sampling_transfer_proof

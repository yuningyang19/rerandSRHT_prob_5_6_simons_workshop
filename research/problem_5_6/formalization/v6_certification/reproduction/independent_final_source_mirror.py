...
R=pathlib.Path('/Users/yuningyang/GitHub/rsvd_essential_v6_certification');E=R/'research/problem_5_6/formalization/v6_certification';archive=E/'packages'/archive_name;dest=E/'packages'/('cold-'+slug);prior_report=E/'reproduction'/('independent_final_source_cold_'+slug+'-resumed.json');report=E/'reproduction'/('independent_final_source_cold_'+slug+'-source-mirror.json');log=report.with_suffix('.log')
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest();utc=lambda:datetime.datetime.now(datetime.timezone.utc).isoformat()
assert sha(archive)==expected_sha;assert dest.exists();assert not report.exists();prior=json.loads(prior_report.read_text());assert prior['status']=='FAIL' and prior['initial_compiled_objects']==0 and not prior['changed_proof_or_build_inputs'];assert not any(x['command'][-1].endswith('/scripts/run_suite.py') for x in prior['execution_commands']);started=time.monotonic();record={'status':'RUNNING','started_utc':utc(),'verification_commit':commit,'archive_sha256':expected_sha,'archive':str(archive),'destination':str(dest),'destination_was_absent':False,'destination_was_absent_in_initial_attempt':prior['destination_was_absent_in_initial_attempt'],'prior_source_acquisition_failure_report':str(prior_report),'prior_source_acquisition_failure_sha256':sha(prior_report),'execution_method':'After two preserved pre-compilation official Mathlib transport failures, revalidate the same new archive directory and zero compiled objects; fetch exact SOURCE Git objects only from the independently verified official-source repositories in the c030 cold tree, without copying their working trees or compiled/build artifacts. Verify exact clean HEADs and zero compiled objects again, then invoke the corrected archive documented run_suite.py exactly once. This uses cached source Git objects and is not an unqualified cache-free run; every proof object is compiled from source in the new tree.','compiled_cache_reused':False,'source_object_mirror_reused':True,'global_git_config_modified':False,'source_fetches':[]}
...
try:
 ...
 with log.open('wb') as out:
  for dep in dependencies['packages']:
   ...
   mirror=E/'packages/cold-c0303801ca01-shallow/research/problem_5_6/formalization/lean/.lake/packages'/dep['name']
   assert subprocess.check_output(['/usr/bin/git','rev-parse','HEAD'],cwd=mirror,text=True).strip()==dep['rev']
   assert subprocess.check_output(['/usr/bin/git','remote','get-url','origin'],cwd=mirror,text=True).strip()==dep['url']
   assert not subprocess.check_output(['/usr/bin/git','status','--porcelain','--untracked-files=no'],cwd=mirror,text=True).strip()
   run(['/usr/bin/git','-c','gc.auto=0','fetch','--depth','1',str(mirror/'.git'),dep['rev']],d,out);run(['/usr/bin/git','checkout','--detach','FETCH_HEAD'],d,out);head=subprocess.check_output(['/usr/bin/git','rev-parse','HEAD'],cwd=d,text=True).strip();assert head==dep['rev'];dirty=subprocess.check_output(['/usr/bin/git','status','--porcelain','--untracked-files=no'],cwd=d,text=True).strip();assert not dirty;record['source_fetches'].append({'name':dep['name'],'url':dep['url'],'expected_revision':dep['rev'],'actual_revision':head,'tracked_source_clean':True,'source_only_shallow_fetch':True,'source_mirror_git_dir':str(mirror/'.git'),'source_mirror_official_origin_verified':True,'no_working_tree_or_build_copy':True});save()
